  ...
  <div class="ajustes-cor-formulario">
    ...
    <form action="compra-update">
      <fieldset class="caixa_formulario">
        <input type="hidden" id="id_compra" name="id_compra" class="form-control" value="${compra.id}" />
        <div class="campo">
          <legend>Data da compra</legend>
          <input type="text" placeholder="Digite a data da compra" id="data_compra" name="data_compra" class="form-control" value="${compra.data_compra}">
        </div>
        <div class="campo">
          ...
          <select id="cliente" name="id_cliente" class="form-control">
            <c:forEach items="${listaClientes}" var="u">
              <c:choose>
                <c:when test="${u.id_cliente eq compra.c1.id_cliente}">
                  <option value="${u.id_cliente}" selected>${u.nome}</option>
                </c:when>
                <c:otherwise>
        ...
        <div class="campo">
          ...
          <select id="passagem" name="id_passagem" class="form-control">
            <c:forEach items="${listaPassagens}" var="p">
              <c:choose>
                <c:when test="${p.id_passagem eq compra.p1.id_passagem}">
                  <option value="${p.id_passagem}" selected>${p.cidade}</option>
                </c:when>
                <c:otherwise>
        ...
        <div class="campo">
          ...
          <input type="text" placeholder="Cartão ou dinheiro" id="metodo_pagamento" name="metodo_pagamento" class="form-control" value="${compra.metodo_pagamento}">
        </div>
        <input type="submit" value="Enviar">
        <a href="./compra" class="btn btn-danger" style="margin-left: 10px">Cancelar</a>
      </fieldset>
    </form>
  </div>
</main>
